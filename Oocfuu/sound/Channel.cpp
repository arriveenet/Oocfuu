...
void Channel::update()
{
	...
	if (m_length >= phaseLength) {
		...
		// ãxïÑà»äOèÍçá
			...
			//audioWaveform(m_channel, m_waveform);
			audioFreq(m_channel, m_score[m_phase].freq);	// é¸îgêîÇê›íË
			audioDecay(m_channel, m_score[m_phase].decay);	// å∏êäÇê›íË
			audioGain(m_channel, m_gain);					// âπó Çê›íË
